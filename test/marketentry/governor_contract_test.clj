  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Vaduz Digital Services AG"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Vaduz Digital Services AG" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest fiscal-rep-missing-is-held-and-unoverridable
  (testing "missing Liechtenstein fiscal representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:fiscal-rep-missing} (-> (store/ledger db) last :basis)))
...
(deftest peid-unverified-is-held-and-unoverridable
  (testing "unverified PEID when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:peid-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
