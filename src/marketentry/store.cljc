(ns marketentry.store
  "SSoT for the LIE market-entry compliance actor, behind a `Store`
  ...
  `:requires-fiscal-rep?`/`:has-fiscal-rep?` and
  `:requires-peid?`/`:peid-verified?` are this vertical's own ground
  truth (see `marketentry.governor` for the citations grounding them --
  Liechtenstein's Steuerverwaltung-administered VAT/fiscal-representative
  requirement and the PEID/FL-UID tax-identifier system, NOT copied
  verbatim from CHE's `:requires-ch-presence?`/`:requires-uid?` fields).
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Vaduz Digital Services AG" :portal "vergaben.llv.li"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-fiscal-rep? true :has-fiscal-rep? true
             :requires-peid? true :peid-verified? true
             :drafted? false :submitted? false
             :jurisdiction "LIE" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "vergaben.llv.li"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-fiscal-rep? true :has-fiscal-rep? true
             :requires-peid? true :peid-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Rheinstahl Consulting Anstalt" :portal "vergaben.llv.li"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "LIE" :status :intake}
    "eng-4" {:id "eng-4" :operator "Alpenland Export Treuhand" :portal "vergaben.llv.li"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-fiscal-rep? true :has-fiscal-rep? false
             :requires-peid? true :peid-verified? true
             :drafted? false :submitted? false
             :jurisdiction "LIE" :status :intake}
    "eng-5" {:id "eng-5" :operator "Malbun Logistics Establishment" :portal "vergaben.llv.li"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-fiscal-rep? true :has-fiscal-rep? true
             :requires-peid? true :peid-verified? false
             :drafted? false :submitted? false
             :jurisdiction "LIE" :status :intake}
    "eng-6" {:id "eng-6" :operator "Triesenberg Civic Tech AG" :portal "vergaben.llv.li"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-fiscal-rep? has-fiscal-rep?
                               requires-peid? peid-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-fiscal-rep?)   (assoc :engagement/requires-fiscal-rep? requires-fiscal-rep?)
    (some? has-fiscal-rep?)        (assoc :engagement/has-fiscal-rep? has-fiscal-rep?)
    (some? requires-peid?)   (assoc :engagement/requires-peid? requires-peid?)
    (some? peid-verified?)   (assoc :engagement/peid-verified? peid-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-fiscal-rep? :engagement/has-fiscal-rep?
   :engagement/requires-peid? :engagement/peid-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-fiscal-rep? (boolean (:engagement/requires-fiscal-rep? m))
     :has-fiscal-rep? (boolean (:engagement/has-fiscal-rep? m))
     :requires-peid? (boolean (:engagement/requires-peid? m))
     :peid-verified? (boolean (:engagement/peid-verified? m))
