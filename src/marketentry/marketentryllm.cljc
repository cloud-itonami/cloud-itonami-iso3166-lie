(ns marketentry.marketentryllm
  ...
  the LIE public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-fiscal-rep?=" (:has-fiscal-rep? e)
                        " peid-verified?=" (:peid-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-fiscal-rep? e))
                              (:has-fiscal-rep? e))
                          (or (not (:requires-peid? e))
                              (:peid-verified? e)))
                   0.9 0.3)}))
